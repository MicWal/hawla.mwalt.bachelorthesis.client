package de.landshut.haw.edu.control.algorithm;

import de.landshut.haw.edu.model.environment.SoccerEnvironment;

public class SoccerPatternAlgorithm {
		
	
	private SoccerEnvironment environment;
	
	
	public SoccerPatternAlgorithm() {
		
	}
	
	
	public void checkPattern(SoccerEnvironment environment) {
		
		this.environment = environment;
		
	}
	// load existing patterns
	
	// get current environment as picture
	
	// is environment picture similar to a picture of a beginning pattern; add to possible matches array with timestamp
	
	
		
	// match -> add new pattern
	
	// environment doesn't match anymore -> 
	
}
